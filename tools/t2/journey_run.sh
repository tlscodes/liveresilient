...
FIXTURES=${JOURNEY_FIXTURES:-$REPO/tools/t2/journey_fixtures.sh}
...
# WHITELIST_BLOCKED_HOST is set below, next to WL_SELF, because its default IS
# WL_SELF: the destination must be one the phone can resolve at layer 2.
...
# The relay is restarted from source by this runner (whitelist profile) so the
# binary that answers the door and the log the rendezvous is read from are the
# same process. Overridable so the precondition can be tested without a relay.
RELAY_RESTART=${JOURNEY_RELAY_RESTART:-$REPO/tools/t2/relay_restart.sh}
...
# The destination of BOTH negative controls (TCP 443 must be reset, UDP 443
# must get nothing). It has to be an address whose SYN actually reaches pf on
# $IFACE, or the control measures an unreachable host instead of the filter.
# The earlier default 192.168.2.9 did not: it is inside the phone's own
# 192.168.2.0/24, so the phone ARPs for it, nothing answers (the Mac does not
# proxy-ARP), no SYN is ever emitted, and the connect ends EHOSTUNREACH — the
# same outcome on a filtered and on an unfiltered network.
# $WL_SELF is the Mac's own address on $IFACE: it answers ARP itself, so the
# SYN is guaranteed to arrive at pf on $IFACE, and $WHITELIST_PORT_443 is NOT
# in $WHITELIST_TCP, so the packet passes none of the allow rules and falls to
# `block return-rst in quick on $IFACE proto tcp from $PEER to any port 443`
# (net_shape.sh:558) — a real RST, generated by the filter under test.
# An off-link decoy (an address outside 192.168.2.0/24 that the phone's default
# route sends to this Mac, e.g. 203.0.113.9) is the other faithful choice and
# can be set here; it is not the default because it also asserts that the phone
# prefers the bridge for its default route, which this Mac cannot verify, while
# an on-link address on $IFACE is reachable by construction.
WHITELIST_BLOCKED_HOST=${JOURNEY_WHITELIST_BLOCKED_HOST:-$WL_SELF}
# Named in both rows through $SCOPE: which destination the reset control aimed
# at and why that destination reaches the filter at all. The guard below refuses
# the one shape that cannot work, so the env override cannot reintroduce it.
WHITELIST_BLOCKED_NOTE=""
if [ "$PROFILE" = whitelist ]; then
  wl_blocked_why="on-link on $IFACE (this Mac answers its own ARP), so the SYN reaches pf"
  if [ "$WHITELIST_BLOCKED_HOST" != "$WL_SELF" ]; then
    if [ "${WHITELIST_BLOCKED_HOST%.*}" = "${PEER%.*}" ]; then
      # Inside the phone's /24 (the shape Internet Sharing gives this bridge).
      # Then it is only usable if this Mac answers its ARP, which means the
      # address is configured on $IFACE (`ifconfig $IFACE alias <addr>/32`).
      # Held by nobody, it is the 2026-09-05 defect: the phone emits no SYN at
      # all and the control can only report an absent host.
      if ifconfig "$IFACE" 2>/dev/null | grep -q "inet $WHITELIST_BLOCKED_HOST "; then
        wl_blocked_why="an alias on $IFACE (this Mac answers its ARP), so the SYN reaches pf"
      else
        die "the blocked host $WHITELIST_BLOCKED_HOST sits in the phone's own ${PEER%.*}.0/24 and no address on $IFACE answers its ARP: the phone would never put a SYN on the wire, so the reset control would measure an unreachable host instead of the filter — use $WL_SELF, add it as an alias on $IFACE, or pick an address outside ${PEER%.*}.0/24 that the phone's default route sends to this Mac"
      fi
    else
      wl_blocked_why="off-link, operator-set: the phone's default route must send it to this Mac, or the SYN never reaches pf"
    fi
  fi
  WHITELIST_BLOCKED_NOTE="reset_control=$WHITELIST_BLOCKED_HOST:$WHITELIST_PORT_443 ($wl_blocked_why; port $WHITELIST_PORT_443 is outside the allow list {$WHITELIST_TCP}, so block return-rst answers it)"
fi
...
if [ "${JOURNEY_DRY:-0}" = 1 ]; then
  ...
  echo "dry       $WHITELIST_BLOCKED_NOTE"
  echo "relay     RELAY_LOG=$RELAY_LOG $RELAY_RESTART $RELAY_PORT   (restarted from source, so the binary that answers and the log the rendezvous is read from are one process)"
  echo "relay     curl -sk -o /dev/null -w %{http_code} --max-time 5 https://127.0.0.1:$RELAY_PORT/ must print 200; a relay from before the ordinary-service page answers 426 and would burn the whole run"
...
# --- the relay precondition, BEFORE the fixtures, pf, the build and the phone ---
# For every other profile a non-answer is only a note (further down): those rows
# never read the relay's log, and a WSS-only relay serves them fine. The
# whitelist profile is different. Its door row needs the ordinary HTTPS page
# (status 200 on a non-upgrade GET of `/`) and its rendezvous row needs the
# `room_rendezvous_complete` line in $RELAY_LOG. A relay still running from
# before those two changes answers 426 with an empty body and logs neither --
# and `curl -sk ... >/dev/null` exits 0 for ANY status, so that stale process
# used to pass the old check and be found only after the pf load, a ~20 min
# build, the phone launch and a whole call, with no door_open event and no
# rendezvous witness. So this profile restarts the relay from source here
# (which is also what makes $RELAY_LOG and the answering process one and the
# same) and then requires the literal status 200, before anything is built.
if [ "$PROFILE" = whitelist ]; then
  [ -x "$RELAY_RESTART" ] || die "the whitelist profile restarts the relay from source, but $RELAY_RESTART is missing or not executable"
  relay_out=$(RELAY_LOG="$RELAY_LOG" "$RELAY_RESTART" "$RELAY_PORT" 2>&1) \
    || die "the relay would not restart from source: $(printf '%s' "$relay_out" | tail -1) (see $RELAY_LOG)"
  printf '%s\n' "$relay_out" | sed 's/^/relay     /'
  relay_status=$(curl -sk -o /dev/null -w '%{http_code}' --max-time 5 "https://127.0.0.1:$RELAY_PORT/" 2>/dev/null)
  [ "$relay_status" = 200 ] || die "the relay on $RELAY_PORT answered ${relay_status:-nothing} to a non-upgrade GET / and the whitelist profile needs the ordinary-service page (200): 426 is a process from before that change, which would also log no room_rendezvous_complete, so both rows would lose their witness after an hour of rig time (log $RELAY_LOG)"
  echo "relay     ordinary-service page 200 on https://127.0.0.1:$RELAY_PORT/  log $RELAY_LOG  (binary restarted from source, so the log matches it)"
fi
...
# Every other profile only needs something to answer on the relay port; the
# whitelist profile's own, much stricter precondition ran further up, before
# the fixtures, because a stale relay there costs a whole rig run.
if [ "$PROFILE" != whitelist ]; then
...
if [ "$PROFILE" = whitelist ]; then
  ...
  SCOPE="whitelist on $IFACE: tcp {$WHITELIST_TCP} + udp 53 to $WL_SELF, tcp $WHITELIST_PORT_443 elsewhere reset, all else from $PEER dropped; $WHITELIST_BLOCKED_NOTE"
