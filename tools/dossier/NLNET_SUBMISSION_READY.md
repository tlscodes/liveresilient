...
Every design decision, measurement and acceptance criterion is mine. Every
number in this application traces to a committed artifact rather than to a
model's recollection; I checked that figure by figure, and where a figure could
not be traced it was cut rather than softened - including two engineering
numbers that had been carried forward in prose and turned out to exist in no
results file. The repository also carries a number-source lint for prose lines;
it is a working tool, not the proof of this text.
```
