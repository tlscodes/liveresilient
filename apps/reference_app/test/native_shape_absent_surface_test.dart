...
bool _buildsStatusInline(String source) =>
    RegExp(r'NativeShapeAbsent\s*\(\s*(?![:)])(?![a-z]\w*\s*:)')
        .hasMatch(_codeOnly(source));

List<File> _presentationFiles() {
  final root = Directory('lib');
  if (!root.existsSync()) {
    throw StateError('run from apps/reference_app; cwd is '
  ...
  group('4e the absent state is on screen, derived not authored', () {
    testWidgets('4e the panel renders the value\'s own text, unconditionally',
      ...
      for (final panel in [
        ...
        expect(
          find.text(expected),
          findsOneWidget,
          reason: 'the panel must render the status value\'s own words, with no '
    ...
    test('4e every cause states unavailability and claims nothing', () {
      ...
      for (final cause in NativeShapeAbsentCause.values) {
        ...
        expect(text.trim(), isNotEmpty, reason: '${cause.name} renders nothing');
        expect(
          text.toLowerCase(),
        ...
        expect(
          text.toLowerCase(),
          isNot(anyOf(contains('coming soon'), contains('will be'),
              contains('soon'), matches(RegExp(r'\b20\d\d\b')))),
  ...
  group('4f the wording and the state have exactly one home', () {
    ...
    test('4f no surface constructs the status value itself', () {
      ...
      expect(
        offenders,
        isEmpty,
        reason: 'a surface that builds its own status decides for itself what '
    ...
    test('4f the panel takes the value from the package constant', () {
      final panel =
          File('lib/src/ui/diagnostics_panel.dart').readAsStringSync();
      ...
      expect(panel.contains('NativeShapeAbsentCause.'), isFalse,
          reason: 'branching on the cause at the surface would put the wording '
              'decision back in the panel');
    });

    test('4f the wording owner derives from the type, with no default branch',
        () {
      ...
      expect(RegExp(r'^\s*_\s*=>', multiLine: true).hasMatch(code), isFalse,
