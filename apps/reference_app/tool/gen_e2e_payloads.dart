...
void main() {
  ...
  void emit(String name, Uint8List bytes, int mustBe, String source) {
    if (bytes.length != mustBe) {
      throw StateError(
        '$name is ${bytes.length}B, phase-5 source says '
  ...
  final corpus = File(
    '$_repo/tools/phase5/corpus/chat_corpus_200.jsonl',
  ).readAsLinesSync();
  Uint8List? textWire;
  int? textId;
  ...
  final page =
      jsonDecode(
            File(
              '$_repo/tools/phase5/corpus/news_ref_page.json',
            ).readAsStringSync(),
          )
          as Map<String, Object?>;
  emit(
    'news_1160b.bin',
    encodeNewsPage(page, _brotliCli),
    1160,
  ...
  try {
    final wav = File(
      '$_repo/tools/phase5/corpus/speech_ref_10s.wav',
    ).readAsBytesSync();
