...
- **UPDATE 2026-07-17 — security slice landed:** structured-mutation parser fuzzing (tool/fuzz CLI + per-package 12k-iteration regression suites) — 6 targets × 50k iterations, 0 non-contract exceptions after fixing one REAL int64-overflow bug the fuzzer found in mesh frame bounds (`media_frame.dart`); `security/SBOM.md` (51 deps, regeneration command); dep report: all direct deps current. Independent audit/pen-test + osv network scan remain open (external/CI).
