...
- **STATUS 2026-07-17 — Xcode slot executed; only the 2-device item remains:** Xcode 26.3 installed+active; `apps/reference_app` built green on macOS with real `flutter_webrtc` via `FlutterWebRtcPeerConnectionPort` (packages/media_webrtc_flutter); REAL-media loopback E2E green on one machine (realAudio: packetsReceived rising on both peers [5,41,61,82]→[86,105,126,146] across ICE restart, clean hangup, relay rooms drained) and 100× setup/teardown cycles with 0 errors + negative steady-state RSS growth (no leak) — the G3 loopback gate items are closed. **1 dated blocker (2026-07-17):** real 2-device call + TURN fallback on devices (G4) needs physical phones; scheduled for the device slot.
...
- **STATUS 2026-07-15 — pure-Dart scope CLOSED (419 tests green):** real Ed25519 everywhere (identity engine, manifest verifier + signer/keygen CLIs, envelope/link-frame auth), TURN short-lived credentials (known-vector pinned), threat model T1-T19 + DATA_FLOW.md complete. All tamper/replay tests reject 100%. **UPDATE 2026-07-17 — Keychain adapter DONE:** `packages/security_keychain` `KeychainKeyMaterialStore` (flutter_secure_storage, accessibility first_unlock_this_device, no-backup-migration) proven on the REAL macOS Keychain (5 integration tests: persistence across re-instantiation, idempotent delete, strict null, overwrite; +8 unit). Data-Protection keychain flavor requires a real signing identity (documented; ad-hoc test host used the login-keychain flavor of the same plugin path). **Remaining dated blocker (2026-07-17):** Android Keystore half needs an Android device/emulator → device slot. Dev key stores remain loudly dev-only.
...
- **STATUS 2026-07-17 — iOS simulator slice ONLY:** ios/ platform added to reference_app (Podfile ios 13.0 per flutter_webrtc podspec; Info.plist mic/camera usage strings + UIBackgroundModes [audio, voip] as CallKit/PushKit prep — native Swift deferred by design); `flutter build ios --simulator --debug` ✓; loopback smoke on iPhone 17 sim (iOS 26.3.1): real wss TLS signaling + real WebRTC ICE/DTLS connected both ways, ICE restart recovered, clean hangup, rooms drained — RTP flow deferred (sim has no mic; macOS realAudio run re-verified green same night). **Everything else in this phase is device-bound (dated 2026-07-17):** CallKit/PushKit native integration, Android platform + Keystore, all 2-device/BT/lock-screen/cellular gates → device slot. Phase 9 is OPEN.
...
- **UPDATE 2026-07-17 — security slice landed:** structured-mutation parser fuzzing (tool/fuzz CLI + per-package 12k-iteration regression suites) — 6 targets × 50k iterations, 0 non-contract exceptions after fixing one REAL int64-overflow bug the fuzzer found in link frame bounds (`media_frame.dart`); `security/SBOM.md` (51 deps, regeneration command); dep report: all direct deps current. Independent audit/pen-test + osv network scan remain open (external/CI).
...
- Any specialized external transport module is **out of core** — if ever built, it is a
  separate, independently reviewed repository, not part of this plan.
