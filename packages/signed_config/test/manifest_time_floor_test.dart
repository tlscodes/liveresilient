...
void main() {
  group('persisted time floor', () {
    ...
    test('5a  a device clock one day behind issuedAt accepts with the floor, '
        'and is rejected as not-yet-valid without it', () async {
    ...
    test('5d  the floor never extends a document past its expiresAt, and never '
        'reaches back before its issuedAt', () async {
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          drifts.length,
          reason:
              'the floor must make the rollback branch reachable on every '
    ...
    group('5e verifyLenient — the single relaxation point', () {
      test('5e a strictly valid document comes back LenientAccepted, '
          'not stale', () async {
      ...
      test('5e an authentic document failing only as expired is accepted '
          'stale with its cause named', () async {
        final result = await verifier.verifyLenient(
          doc(),
          lastAcceptedRevision: 41,
      ...
      test('5e an authentic document failing only as notYetValid is accepted '
          'stale with its cause named', () async {
        final result = await verifier.verifyLenient(
          doc(),
          lastAcceptedRevision: 41,
      ...
      test('5e a tampered signature is never relaxed, even when the document '
          'is also expired', () async {
        final tampered = signManifest(
          buildManifest(revision: 42, issuedAt: issuedAt, expiresAt: expiresAt),
          key1,
        ...
        expect(
          ...
          reason:
              'relaxation may only touch the two time facts; '
              'authenticity failures are final',
        );
      });

      test('5e an unknown signing key is never relaxed', () async {
      ...
      test('5e a rollback is never relaxed: the relaxed pass re-checks the '
          'revision and reports it', () async {
        ...
        expect(result, isA<LenientRejected>());
        expect((result as LenientRejected).reason, ManifestRejection.rollback);
      });
    });
  });
}
