...
EndpointManifest _manifest(List<IceServerEntry> ice, {Map<String, bool>? flags}) =>
    EndpointManifest(
...
void main() {
  group('buildRtcIceConfig', () {
    ...
    test('omits empty credential keys rather than sending blanks', () {
      final cfg = buildRtcIceConfig(_manifest([_e(['stun:stun.example:3478'])]));
      expect(cfg.iceServers.single.containsKey('username'), isFalse);
      expect(cfg.iceServers.single.containsKey('credential'), isFalse);
    });

    test('orders turns-443 first, then turns, turn, stun — and drops nothing', () {
      final cfg = buildRtcIceConfig(_manifest([
      ...
      expect(cfg.iceServers, hasLength(4), reason: 'ordering must not filter');
    ...
    test('strictRelay forces relay policy and drops stun-only entries', () {
      ...
      expect(cfg.iceServers, hasLength(1));
      expect((cfg.iceServers.single['urls']! as List).single,
    ...
    test('an entry carrying both stun and turn urls survives strictRelay', () {
      final cfg = buildRtcIceConfig(
        _manifest([
          _e(['stun:relay.example:3478', 'turns:relay.example:443'],
              user: 'u', cred: 'c'),
        ]),
        profile: IceProfile.strictRelay,
      );
  ...
  group('iceProfileFor', () {
    test('normal by default — relaying everything is never the default', () {
      expect(iceProfileFor(iceFailureCount: 0, featureFlags: const {}),
          IceProfile.normal);
      expect(iceProfileFor(iceFailureCount: 1, featureFlags: const {}),
          IceProfile.normal);
    });

    test('two failures on the same call escalate to relay-only', () {
      expect(iceProfileFor(iceFailureCount: 2, featureFlags: const {}),
          IceProfile.strictRelay);
    });

    test('the manifest flag can force it regardless of failures', () {
      expect(
        iceProfileFor(iceFailureCount: 0, featureFlags: const {'strict_relay': true}),
        IceProfile.strictRelay,
      );
    });
  });
