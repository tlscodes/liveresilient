/// Structured-mutation regression coverage for `package:signed_config`'s
/// untrusted-input parsers.
///
/// Duplicates the target logic in `tool/fuzz/targets_signed_config.dart`
/// (see `test/support/fuzz_support.dart` for why it's a copy, not a shared
/// dependency) so `dart test` alone proves the contract: every target
/// either returns a valid object or throws FormatException/ArgumentError.
/// Any other exception is a regression and fails the test with a minimal,
/// seed-based repro.
library;
...
Map<String, Object?> _buildValidManifestTree(FuzzRng rng) {
...
class _EndpointManifestFuzzTarget extends FuzzTarget {
  @override
  String get name => 'endpoint_manifest';

  @override
  FuzzCase generate(FuzzRng rng) {
    final valid = _buildValidManifestTree(rng);
...
void main() {
  group('parser robustness (structured-mutation fuzzing)', () {
    _runAndAssert(_SignedManifestDocumentFuzzTarget());
    _runAndAssert(_EndpointManifestFuzzTarget());
  });
}
