...
void main() {
  group('5e embedded time floor + lenient re-check (cache level)', () {
    ...
    test(
      '5e fresh install: a document older than the embedded floor is '
      'rejected even when the device clock sits inside its window',
      () async {
    ...
    test(
      '5e fresh install: the embedded floor never rejects a document '
      'that is current relative to the floor',
      () async {
    ...
    test(
      '5e the lenient re-check extends to notYetValid on the persisted '
      ...
      () async {
        ...
        expect(
          ...
          42,
          reason: 'before 5e only expired was relaxed at startup; a '
    ...
    test(
      '5e relaxation on the freshly-fetched path never admits an '
      'inauthentic document, however expired',
      () async {
        final issuedAt = floor.add(const Duration(days: 10));
        final expiresAt = issuedAt.add(const Duration(days: 1));
        final manifest = buildManifest(
        ...
        await expectLater(
          newCache().get(),
          throwsA(isA<ManifestUnavailable>()),
        );
