...
void main() {
  ...
  group('serving', () {
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          'first',
          reason:
              'expiry must never stand between the caller and a usable '
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          'first',
          reason:
              'a failed refresh is not a positive signal, so the entry '
              'stays in service',
        );
      },
    );
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          'recovered',
          reason:
              'the key must be usable again on the very next call; a '
    ...
    test(
      '6e  a null upstream answer is not cached and evicts nothing',
      () async {
        var answer = 'first';
        final cache = LookupCache(
          (key) async => answer,
  ...
  group('gate 6d — expired cache, network down', () {
    test('6d  the attempt is made with the stale record, '
      ...
      expect(
        ...
        '203.0.113.7',
        reason:
            'expired + network down must hand the caller the stale '
      ...
      expect(refreshThrew, isTrue, reason: 'and it must actually have failed');
  ...
  group('the three positive signals', () {
    ...
    test('6e  a caller reporting the answer did not work drops it', () async {
      ...
      expect(
        ...
        'v2',
        reason:
            'a reported failure is a positive signal and drops the entry '
            'even while it is still fresh',
      );
    });

    test(
      '6e  an epoch advance drops entries from the previous epoch',
      () async {
        var calls = 0;
        final cache = LookupCache(
          (key) async {
  ...
  group('bounds', () {
    test('6e  only one refresh is in flight per key', () async {
      ...
      expect(
        ...
        1,
        reason:
            'three simultaneous misses on one key must produce one '
            'request, not three',
      );
    });

    test(
      '6e  memory is bounded: the least recently used entry is evicted',
      () async {
        var calls = 0;
        final cache = LookupCache(
          (key) async {
        ...
        expect(
          calls,
          before + 1,
          reason: 'b was evicted and had to be re-asked',
        );
      },
    );
