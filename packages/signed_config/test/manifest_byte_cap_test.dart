...
void main() {
  group('manifest byte cap', () {
    ...
    test('3d  the ice-server ceiling is 256, single document, one signature', () {
      ...
      expect(
        ...
        reason: 'one document carrying one signature; no page index, no '
    ...
    test(
      '3d-bis  a document over the cap is refused before it is parsed',
      () {
        ...
        expect(
          ...
          reason: 'the bytes here are not even valid JSON — refusing them by '
    ...
    test(
      '3d-bis  the cap covers the persisted-document path, not only the '
      'network path',
      () async {
    ...
    test(
      '3d-bis  a document at exactly the cap is still accepted',
      () async {
    ...
    test(
      '3d-bis  the cap matches the fetcher default, so a document accepted '
