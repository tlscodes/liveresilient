...
void main() {
  test('photo + flipbook + document during a 120s hostile-channel call; '
    ...
    final vidFrames = List.generate(
      ...
      (t) => Uint8List.fromList(
        List.generate(240 * 160, (i) => (i % 240 + t * 12) & 0xFF),
      ),
    );
    ...
    // ignore: avoid_print
    print(
      'phase5 (simulated): compressed sizes '
  ...
  test('layered photo: a viewable preview lands long before the full '
    ...
    int runToCompletion({required bool layered}) {
      ...
      _voiceTicks(120000, (nowMs, speaking) {
        ...
        for (final d in transport.queue.tick(
          ...
          if (rng.nextDouble() < 0.60 || ge.shouldDrop()) continue;
          final dec = decoders.putIfAbsent(d.transferId, RatelessDecoder.new);
          dec.addDatagram(d.bytes);
    ...
    expect(
      ...
      lessThan(monolithic),
      reason:
          'layering must surface a preview earlier than the '
          'monolithic transfer completes',
    );
    // ignore: avoid_print
    print(
      'layered photo: first viewable at ${(layeredMs / 1000).toStringAsFixed(1)}s vs ${(monolithic / 1000).toStringAsFixed(1)}s '
  ...
  for (final carrier in MediaCarrier.values) {
    test(
      'phase 8 — photo + flipbook + document over the ${carrier.name} '
      ...
      () async {
        ...
        expect(
          transport.tlsParameters.buildCipherSuites().any(
            transport.tlsParameters.isGreaseValue,
          ),
          isTrue,
        );
        ...
        for (var nowMs = 0; nowMs <= 120000; nowMs += 20) {
          ...
          for (final wire in transport.wireTick(
            ...
            if (dec.isComplete &&
                !completedAt.containsKey(carried.transferId)) {
        ...
        expect(
          ...
          3,
          reason:
              'photo, flipbook and document must ALL complete in 120s '
        ...
        // ignore: avoid_print
        print(
          'phase8 (simulated, ${carrier.name}): compressed photo=$sPhoto '
        ...
        expect(
          ...
          reason:
              'length normalization and framing may cost bytes, but the '
