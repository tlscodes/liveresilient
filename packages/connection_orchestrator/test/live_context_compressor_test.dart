...
void main() {
  ...
  test('round-trip is byte-exact: text, Persian, binary, random, edges', () {
    ...
    for (final data in cases) {
      expect(c.decompress(c.compress(data)), equals(data),
  ...
  test('random (incompressible) data expands by less than 1%', () {
    final rng = Random(4);
    final data =
        Uint8List.fromList(List.generate(16384, (_) => rng.nextInt(256)));
  ...
  test('measured head-to-head vs gzip level 9 (must win on every corpus)',
    ...
    final corpora = <String, Uint8List>{
      '10KB representative doc':
          Uint8List.fromList(utf8.encode(_representativeText(10240, 42))),
      '10KB Persian prose': Uint8List.fromList(utf8.encode(
          ('در سکوت، داده‌ها منتقل می‌شوند و صدا همیشه مقدم است؛ '
                  'سند و عکس در پس‌زمینه، ذره‌ذره، بدون بازخورد. ')
              .padRight(1, ' ') *
              120)),
      '16KB dart source': Uint8List.fromList(utf8.encode(
