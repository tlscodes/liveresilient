  ...
  group('LayeredRedundancyAllocator', () {
    test('pins blockSize 55: 60-byte datagrams, floor under 10%', () {
      ...
      expect(
        ...
        lessThan(0.10),
        reason:
            'framing floor 5/55 = 9.09..% is what makes the gate '
            'reachable; 5/48 = 10.4% is not',
      );
      expect(
        () => LayeredRedundancyAllocator(blockSize: 56),
        throwsArgumentError,
      );
    });
    ...
    test('cold-start law reproduces the measured anchors within tolerance', () {
      ...
      expect(
        LayeredRedundancyAllocator.coldFactor(0.2, isL0: true),
        greaterThanOrEqualTo(1.5),
      );
      expect(
        LayeredRedundancyAllocator.coldFactor(0.5, isL0: true),
        greaterThanOrEqualTo(2.5),
      );
      expect(
        LayeredRedundancyAllocator.coldFactor(0.7, isL0: true),
        greaterThanOrEqualTo(4.0),
      );
      expect(
        LayeredRedundancyAllocator.coldFactor(0.9, isL0: true),
    ...
    test('send factor rises with estimated loss — never fixed', () {
      ...
      for (final loss in [0.0, 0.2, 0.4, 0.6, 0.8]) {
        ...
        expect(
          ...
          greaterThan(prev),
          reason:
              'factor must be strictly increasing in loss (F-2: a '
    ...
    test('under budget pressure L0 is funded first, refinement degrades', () {
      ...
      final l01 =
          (rich.layers[0].sendCount + rich.layers[1].sendCount) *
          a.datagramBytes;
      ...
      expect(
        tight.layers[0].fullyProtected,
        isTrue,
        reason: 'L0 protection is never traded for refinement',
      );
      expect(tight.layers[1].fullyProtected, isTrue);
      expect(
        tight.layers[3].sendCount,
        0,
        reason:
            'a layer that cannot even receive blockCount datagrams is '
    ...
    test('generalizes between the anchor loss values (35%, 60%)', () {
      ...
      for (final loss in [0.35, 0.6]) {
        ...
        expect(
          ...
          reason:
              'L0 must decode between the measured anchors too '
              '(loss $loss)',
        );
      }
    });
