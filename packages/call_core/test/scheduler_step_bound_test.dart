...
void main() {
  ...
  AdaptiveConnectionBudget budgetFor(int rttMs, double loss) =>
      AdaptiveConnectionBudget.fromConditions(
        NetworkConditions(rtt: Duration(milliseconds: rttMs), loss: loss),
      );

  group('maxSchedulerStepFor', () {
    test(
      '2a  across the whole rtt x loss grid the result is always one of the '
          ...
          for (final loss in losses) {
            ...
            switch (bound) {
              case SchedulerStepAdmissible(:final minStep, :final maxStep):
                expect(minStep, greaterThan(Duration.zero),
    ...
    test(
      ...
      () {
        ...
        expect(
          ...
          reason: 'a clamp here would report a plausible step for a link that '
    ...
    test(
      '2f  the two impossible causes are distinguishable, because the '
      ...
      () {
        ...
        expect(
          ...
          reason: 'ample capacity, long round trip: capacity is not the cause '
    ...
    test(
      'the network term is HALF the round trip, because the interactive '
    ...
    test(
      'the lower bound is the time one frame needs at the spare rate',
      () {
    ...
    test(
      'an inverted interval — both bounds defined but the floor above the '
