...
void main() {
  group('TrafficShapingPolicy.fromLengthHistogram', () {
    ...
    test('1c  rungs follow the observed mass, not the observed variety', () {
      ...
      expect(
        ...
        1350,
        reason:
            'the largest observation must be representable or the longest '
    ...
    test(
      '1c  a sample that proves nothing is refused, not silently accepted',
      () {
        expect(
        ...
        expect(
          ...
          throwsArgumentError,
          reason:
              'zero observations is the same guess wearing a '
              "measurement's name",
        );
        expect(
  ...
  group('config parsing (gate 1c)', () {
    ...
    test('1c  a non-ascending ladder is refused', () {
      ...
      expect(
        () => parse({'lengthBuckets': <Object?>[]}),
        throwsA(isA<ProbeDefenseConfigError>()),
      );
    });

    test(
      '1c  a bad sample fails when the config is read, naming the field',
      () {
        expect(
          () => parse({
    ...
    test(
      '1c  omitting both keeps the previous default, so nothing changes',
      () {
