...
void main() {
  ...
  group('admission across epochs', () {
    ...
    test('two-key admission costs about two X25519 operations — measured as '
      ...
      await pkg_clock.withClock(pkg_clock.Clock.fixed(start), () async {
        ...
        final oneKeyAdmission =
            await RotatingRealityAuthenticator(ring: ring).inspect(helloCurrent);
        final twoKeyAdmission =
            await RotatingRealityAuthenticator(ring: ring).inspect(hello);
        expect(oneKeyAdmission.keysTried, 1,
            reason: 'a hello for the current key must match on the first '
                'trial; more means the short-circuit is broken');
        expect(twoKeyAdmission.keysTried, 2,
            reason: 'a hello for the previous key must match on exactly '
        ...
        for (var round = 0; round < rounds; round++) {
          ...
          roundRatios.add((admissionPhaseUs / iterations) /
              (keygenPhaseUs / referenceOps));
        }
        ...
        // ignore: avoid_print
        print('EVIDENCE relay_admission '
        ...
        expect(admissionPerKeygen, lessThan(6.45),
            reason: 'cleanest admission priced in cleanest keygens: at or '
        ...
        expect(admissionPerKeygen, greaterThan(2.25),
            reason: 'keysTried counts loop passes even when the crypto '
