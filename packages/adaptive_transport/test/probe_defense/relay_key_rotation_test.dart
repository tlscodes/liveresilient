...
Future<RelayKeyEpoch> _epoch(int number) async => RelayKeyEpoch(
      epoch: number,
      keyPair: await _agreement.generateEphemeral(),
    );
...
void main() {
  group('RelayKeyRing construction', () {
    ...
    test('the announcement carries public keys only', () async {
      ...
      expect(announcement.currentPublicKey,
          isNot(ring.current.keyPair.privateKey));
    });
  });

  group('rotation mechanics', () {
    test('rotate promotes next to current and demotes current to previous',
        () async {
      final ring = await RelayKeyRing.generate();
    ...
    test('rotating with no staged next is an error, not a silent no-op',
        () async {
      final ring = RelayKeyRing(current: await _epoch(1));
      expect(
        () => ring.rotate(freshNext: KeyPairBytes(
    ...
    test('the previous key is admissible during grace and not after',
        () async {
      ...
      pkg_clock.withClock(
        ...
        () {
          ...
          expect(ring.admissibleKeys, hasLength(1),
              reason: 'an expired key must stop authenticating');
        },
      );
    });

    test('retireExpired drops the previous key once, then does nothing',
        () async {
    ...
    test('the ring never holds more than three keys', () async {
      ...
      for (var i = 0; i < 5; i++) {
        ring.rotate(freshNext: await _agreement.generateEphemeral());
        final held = [ring.current, ring.previous, ring.next]
            .where((k) => k != null)
            .length;
  ...
  group('admission across epochs', () {
    test('a client on the current key is admitted with no update signal',
        () async {
      final ring = await RelayKeyRing.generate();
    ...
    test('once grace expires, the old key no longer authenticates', () async {
      ...
      await pkg_clock.withClock(
        ...
        () async {
          final result =
              await auth.inspect(TlsClientHello.parseRecord(record));
    ...
    test('the next key is not accepted before its epoch begins', () async {
      ...
      final result = await auth.inspect(TlsClientHello.parseRecord(record));
      expect(result.admitted, isFalse,
          reason: 'accepting it early widens the window a stolen key is '
              'useful in');
    });

    test('an unrelated key is passed through after the bounded trial',
        () async {
      final start = DateTime.utc(2026, 7, 27);
      await pkg_clock.withClock(pkg_clock.Clock.fixed(start), () async {
        final ring = await RelayKeyRing.generate();
        ...
        final result = await auth.inspect(TlsClientHello.parseRecord(
          await _hello(
        ...
        expect(result.admitted, isFalse);
        expect(result.keysTried, lessThanOrEqualTo(RelayKeyRing.maxKeys),
    ...
    test('replay memory survives a rotation', () async {
      ...
      await pkg_clock.withClock(pkg_clock.Clock.fixed(start), () async {
        ...
        expect(replay.decision.reason, RealityRejectReason.replayedHello,
            reason: 'a rotation must not hand an attacker a fresh window');
      });
    });

    test('a browser hello is passed through regardless of the ring',
        () async {
      final ring = await RelayKeyRing.generate();
      final auth = RotatingRealityAuthenticator(ring: ring);
      final result = await auth.inspect(TlsClientHello.parseRecord(
    ...
    test('two-key admission costs about two X25519 operations — which is '
      ...
      await pkg_clock.withClock(pkg_clock.Clock.fixed(start), () async {
        ...
        final hello = TlsClientHello.parseRecord(await _hello(
        ...
        expect(perDecision, lessThan(12000),
            reason: 'two X25519 ops plus one HMAC; measured '
                '${perDecision.toStringAsFixed(1)} us');
        expect(perDecision, greaterThan(1000),
            reason: 'if this ever drops below one scalar multiply, the '
  ...
  group('RelayKeyUpdate frame', () {
    ...
    test('does not collide with the identity-proof frame type', () {
      expect(RelayKeyUpdate.frameType,
          isNot(RealityIdentityProof.frameType));
    });

    test('rejects a short, mistyped, or mis-declared frame', () async {
      final good = RelayKeyUpdate.forRing(await RelayKeyRing.generate())
          .encode();
  ...
  group('RelayKeyStore — the client side of an update', () {
    test('applies a newer epoch and then authenticates against it',
        () async {
      final start = DateTime.utc(2026, 7, 27, 12);
      await pkg_clock.withClock(pkg_clock.Clock.fixed(start), () async {
        final ring = await RelayKeyRing.generate(
        ...
        final stale = await auth.inspect(TlsClientHello.parseRecord(
          await _hello(
        ...
        final fresh = await auth.inspect(TlsClientHello.parseRecord(
          await _hello(
            relayPublicKey: store.publicKey,
            timeSlot: auth.currentTimeSlot,
            randomSeed: 77,
    ...
    test('ignores a replayed or backwards update', () async {
      ...
      expect(store.apply(older), isFalse,
          reason: 'a replayed old update must not pin a client backwards');
      expect(store.publicKey, ring.current.publicKey);

      expect(store.apply(RelayKeyUpdate.forRing(ring)), isFalse,
