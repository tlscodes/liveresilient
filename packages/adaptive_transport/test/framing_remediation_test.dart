...
void main() {
  group('MicroDatagramLane u16 pad trailer (MTU truncation regression)', () {
    ...
    test('blockSize outside [1, maxBlockSize] is rejected up front', () {
      final lane = MicroDatagramLane();
      expect(
        () => lane.encodeWithPadding(Uint8List(4), blockSize: 0),
        throwsArgumentError,
      );
      expect(
        () => lane.encodeWithPadding(
          Uint8List(4),
      ...
      final ok = lane.encodeWithPadding(
        Uint8List(4),
    ...
    test('corrupted trailer throws instead of returning wrong bytes', () {
      ...
      expect(
        () => lane.decodeAndStripPadding(Uint8List.fromList([1])),
        throwsFormatException,
      );
    ...
    test('block-count jitter breaks rigid quantization', () {
      ...
      for (var i = 0; i < 200; i++) {
        blockCounts.add(
          lane.encodeWithPadding(payload, blockSize: 64).length ~/ 64,
        );
      }
      expect(
        blockCounts.length,
        ...
        reason:
            'identical payloads must spread across multiple '
  ...
  group('RFC 9113 handshake framing', () {
    test('connection preface bytes are exact (section 3.4)', () {
      expect(
        ascii.decode(http2ConnectionPreface),
        'PRI * HTTP/2.0\r\n\r\nSM\r\n\r\n',
      );
    ...
    test('SETTINGS ACK is empty; non-empty ACK is FRAME_SIZE_ERROR', () {
      ...
      expect(
        () => Http2SettingsFrame(
          ack: true,
          settings: {Http2SettingsFrame.settingsEnablePush: 0},
        ).encode(),
        throwsArgumentError,
      );
    ...
    test('SETTINGS on a non-zero stream or ragged payload is rejected', () {
      final frame = Http2SettingsFrame(
        settings: {Http2SettingsFrame.settingsEnablePush: 1},
      ).encode();
    ...
    test('HEADERS on stream 0 rejected both directions', () {
      expect(
        () => Http2HeadersFrame(
          streamId: 0,
          headerBlockFragment: Uint8List(1),
        ).encode(),
        throwsArgumentError,
      );
      final frame = Http2HeadersFrame(
