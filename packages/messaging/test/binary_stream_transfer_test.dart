...
Uint8List blob(int length, int seed) => Uint8List.fromList(
...
class JitterLatencyPort implements DataChannelPort {
  ...
  @override
  Future<void> send(List<int> frame) async {
    var delay = baseOneWay + Duration(milliseconds: _random.nextInt(jitterMs));
...
void main() {
  (LossyPort, LossyPort) pair(Random r, {double drop = 0.0, double dup = 0.0}) {
  ...
  test(
    'resume: a second offer skips chunks the receiver already holds',
    () async {
      final r = Random(3);
      ...
      await b1.close();
      unawaited(
        firstAttempt.catchError(
          (_) => const BinarySendResult(resumedChunks: 0, totalChunks: 0),
        ),
      );
      ...
      final transferId = Uint8List.fromList(
        sha256.convert(content).bytes.sublist(0, 16),
      );
      rx2.restorePartial(
        transferId,
        chunks,
      ...
      final received = await rx2.completed.first.timeout(
        const Duration(seconds: 10),
      );
      final result = await resultF;
  ...
  test('backpressure gate scales with the send window: buffered depth below '
  ...
  test('backpressure gate floor holds on a thin window: buffered depth above '
  ...
  test('send budget token bucket: the lane never offers more than its '
  ...
  test('send budget far below one chunk per two seconds still transfers '
