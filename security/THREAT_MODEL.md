...
| T2 | Tampered endpoint config (T) | A1, A2 | Ed25519-signed manifest, keys pinned in build, revision monotonicity (anti-rollback), HTTPS-only fetch | Verification logic (revocation, expiry, rollback) against a fake signer: **implemented**. Real Ed25519 backing (`CryptographyEd25519Verifier`): **landing this wave — verify before merge**, see failing tests below. Build-pinned keys in a shipped app: **not yet built** (no app build exists). Live HTTPS config-serving endpoint: **not yet built** (no `config_service` directory exists in this repo; only an offline signer CLI, see T8) | `packages/signed_config/lib/src/manifest_verifier.dart` — `manifest_verifier_test.dart` (fake-crypto, all pass); `packages/signed_config/lib/src/crypto_ed25519_verifier.dart` — `crypto_ed25519_verifier_test.dart` run 2026-07-16: **2 of 8 tests FAIL** ("accepts a manifest signed by its pinned key", "manifest signed by the newly-rotated-in key is accepted") — a validly-signed manifest is being rejected; root cause not yet diagnosed, do not claim this path works until fixed and green |
...
- `CryptographyEd25519Verifier` accept-path failure (T2): 2 of 8 tests in
  `packages/signed_config/test/crypto_ed25519_verifier_test.dart` fail as
  of 2026-07-16 (run during this review). Not dated as a scheduled-slot
  blocker because it is an active, in-progress bug on this same branch,
  not a scoped-later item — flagging it here so it cannot be merged as
  "done" by mistake.
